...
class DLQManagementService:
    ...
    def connect(self):
        ...
        try:
            self.admin_client = KafkaAdminClient(
                bootstrap_servers=self.bootstrap_servers, client_id="nina-dlq-manager"
            )
            self.logger.info(
                f"DLQ Management Service connected to {self.bootstrap_servers}"
            )
            return True
    ...
    def create_dlq_topic(self):
        ...
        try:
            ...
            # Check if admin client is connected
            if not self.admin_client:
                self.logger.error(
                    "Admin client not initialized. Cannot create DLQ topic."
                )
                return False

            # Create topic (type checker knows admin_client is not None here)
            admin_client = self.admin_client  # type: ignore
            ...
            if result:
                for topic_name, future in result.items():
                    try:
                        ...
                        self.logger.info(
                            f"DLQ topic '{topic_name}' created successfully"
                        )
                        ...
                        self.logger.error(
                            f"Failed to create DLQ topic '{topic_name}': {e}"
                        )
                        return False
    ...
    def analyze_dlq_patterns(self, hours_back=24):
        ...
        for msg in messages:
            ...
            analysis["by_error"][error_type] = (
                analysis["by_error"].get(error_type, 0) + 1
            )

            # By consumer group
            group = msg["consumer_group"]
            analysis["by_consumer_group"][group] = (
                analysis["by_consumer_group"].get(group, 0) + 1
            )

            # Retry distribution
            retry_count = msg["retry_count"]
            analysis["retry_distribution"][retry_count] = (
                analysis["retry_distribution"].get(retry_count, 0) + 1
            )
    ...
    def print_dlq_analysis(self, hours_back=24):
        ...
        for topic, count in sorted(
            analysis["by_topic"].items(), key=lambda x: x[1], reverse=True
        ):
            print(f"  {topic}: {count}")

        print(f"\n❌ By Error Type:")
        for error, count in sorted(
            analysis["by_error"].items(), key=lambda x: x[1], reverse=True
        ):
            print(f"  {error}: {count}")

        print(f"\n👥 By Consumer Group:")
        for group, count in sorted(
            analysis["by_consumer_group"].items(), key=lambda x: x[1], reverse=True
        ):
            print(f"  {group}: {count}")
    ...
    def recover_dlq_messages(self, original_topic, limit=10):
        ...
        try:
            ...
            self.logger.info(
                f"Attempting to recover DLQ messages for topic: {original_topic}"
            )
                ...
                if dlq_data.get("original_topic") == original_topic:
                    ...
                    try:
                        future = producer.send(
                            original_topic, value=original_message, key=original_key
                        )
                        future.get(timeout=10)
                        recovered_count += 1
                        print(
                            f"✅ Recovered message {recovered_count}: {original_topic}"
                        )
