...
class BaseService(ABC):
    ...
    def format_response(
        ...
        response = {"success": success, "timestamp": datetime.utcnow().isoformat()}
    ...
    def handle_exception(
        self, e: Exception, operation: str, user_id: str = None
    ) -> Dict[str, Any]:
        ...
        return self.format_response(
            success=False, message=f"Error in {operation}: {str(e)}", errors=[str(e)]
        )

    def paginate_results(
        self, results: List[Any], page: int = 1, per_page: int = 20
    ) -> Dict[str, Any]:
        """Paginate results"""
        total = len(results)
    ...
    def filter_by_date_range(
        self, query, date_field, start_date: datetime = None, end_date: datetime = None
    ):
