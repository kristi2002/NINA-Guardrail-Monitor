...
class NINAKafkaConsumerV2:
    ...
    def _process_guardrail_event_core(self, event, conversation_id):
        ...
        if db_event:
            ...
            # Send notification for guardrail events (if severity is high enough)
            if db_event:
                try:
                    severity = str(event.get("severity", "info")).lower()
                    # Send notifications for medium, high, and critical severity
                    if severity in ["medium", "high", "critical"]:
                        ...
                        with get_session_context() as session:
                            ...
                            alert_data = {
                                'title': event.get('event_type', 'Guardrail Event'),
                                'severity': severity,
                            ...
                            # Send notification (will use rules, preferences, etc.)
                            notification_result = orchestrator.send_alert_notification(
                                alert_id=str(db_event.id),
                                alert_data=alert_data
                            )
                            
                            if notification_result.get('success'):
                                self.logger.info(
                                    f"Notification sent for guardrail event {db_event.id} "
                                    f"to {notification_result.get('data', {}).get('recipients_count', 0)} recipient(s)"
                                )
                            else:
                                self.logger.warning(
                                    f"Failed to send notification for guardrail event {db_event.id}: "
                                    f"{notification_result.get('message', 'Unknown error')}"
                                )
                except Exception as notification_error:
                    # Don't fail the event processing if notification fails
                    self.logger.error(
                        f"Error sending notification for guardrail event: {notification_error}",
                        exc_info=True
                    )
