...
class KafkaIntegrationService:
    def __init__(self, bootstrap_servers=None, socketio=None, app=None):
        ...
        self.consumer = NINAKafkaConsumerV2(
            self.bootstrap_servers, app=self.app, on_event_callback=self.handle_consumed_event)
    ...
    def start_consumer(self):
        ...
        if self.consumer.start_consumer():
            self.logger.info("Kafka integration service consumer started")
            return True
        return False
    ...
    def start_conversation_monitoring(self, conversation_id):
        ...
        try:
            ...
            self.logger.error(f"Error starting conversation monitoring: {e}", exc_info=True)
            return False
    ...
    def stop_conversation_monitoring(self, conversation_id):
        ...
        try:
            ...
            self.logger.error(f"Error stopping conversation monitoring: {e}", exc_info=True)
            return False

    def send_guardrail_event(self, conversation_id, event_type, severity, message, context=None):
        ...
        try:
            ...
            # Send via producer
            success = self.producer.send_guardrail_event(
            ...
            if success:
                ...
                self.logger.info(f"Guardrail event sent: {event_type} for conversation {conversation_id}")
                return True
    ...
    def send_operator_action(self, conversation_id, action_type, message, reason=None, operator_id=None):
        ...
        try:
            ...
            operator_action = {
                ...
                'timestamp': datetime.now().isoformat(),
                'command': {
                    'type': 'STOP' if action_type == 'stop_conversation' else action_type.upper(),
            ...
            if success:
                # Note: Socket.IO emission will be handled by the consumer callback
                # after the event is successfully processed and saved to database
                self.logger.info(f"Operator action sent: {action_type} for conversation {conversation_id}")
                return True
    ...
    def send_false_alarm_feedback(self, conversation_id, original_event_id, feedback):
        ...
        try:
            ...
            if success:
                self.logger.info(f"False alarm feedback sent for conversation {conversation_id}")
                return True
    ...
    def cleanup_old_conversations(self, max_age_hours=24):
        ...
        try:
            ...
            for conv_id, conv_data in self.active_conversations.items():
                if conv_data['status'] == 'stopped':
                    if 'end_time' in conv_data:
                        age_hours = (
                            current_time - conv_data['end_time']).total_seconds() / 3600
            ...
            if total_cleaned > 0:
                self.logger.info(f"Cleaned up {total_cleaned} old conversations from memory")

            return total_cleaned

        except Exception as e:
            self.logger.error(f"Error cleaning up old conversations: {e}", exc_info=True)
            return 0
    ...
    def close(self):
        ...
        try:
            ...
            self.logger.error(f"Error closing Kafka integration service: {e}", exc_info=True)
