...
class ConversationSession(BaseModel):
    ...
    id = Column(String(50), primary_key=True, index=True)  # Use the string session_id as the PK
    
    patient_id = Column(String(50), nullable=False, index=True)
    
    # Session timing
    session_start = Column(DateTime(timezone=True), nullable=False, default=lambda: datetime.now(timezone.utc), index=True)
    session_end = Column(DateTime(timezone=True), nullable=True)
    session_duration_minutes = Column(Integer, nullable=True)
    
    # Session status
    status = Column(String(20), default='ACTIVE', nullable=False, index=True)  # ACTIVE, COMPLETED, TERMINATED, ESCALATED
    ...
    situation = Column(String(255), nullable=True)  # Renamed from 'title' in some services
    ...
    def patient_name(self):
        ...
        if self.patient_info and isinstance(self.patient_info, dict):
    ...
    def situation_level(self):
        ...
        risk_to_level = {'LOW': 'low', 'MEDIUM': 'medium', 'HIGH': 'high', 'CRITICAL': 'critical'}
        return risk_to_level.get(self.risk_level, 'low')
    ...
    def needs_attention(self):
        ...
        return (self.requires_attention or 
                self.guardrail_violations > 0 or 
                self.is_high_risk())
