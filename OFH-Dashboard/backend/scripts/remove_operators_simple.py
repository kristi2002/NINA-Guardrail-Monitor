#!/usr/bin/env python3
"""
Simple script to remove operator users from the database
Doesn't require dotenv - uses environment variables or defaults
"""
...
# Try to load dotenv if available, otherwise use environment
try:
...
def remove_operator_users():
    ...
    try:
        # Find all users with operator role (including soft-deleted)
        operator_users = session.query(User).filter(User.role == 'operator').all()
        ...
        for user in operator_users:
            logger.info(f"  - {user.username} (ID: {user.id}, Email: {user.email}, Active: {user.is_active}, Deleted: {user.is_deleted})")
        
        # Confirm deletion
        print(f"\n⚠️  Found {len(operator_users)} operator user(s) to delete")
        ...
        operator_by_username = session.query(User).filter(User.username == 'operator').all()
